...
const RiskAnalysis = () => {
  ...
  const handleWhyQuestion = async () => {
    ...
    setIsAskingWhy(true)
    try {
      console.log('XAI Request Data:', {
        question: whyQuestion,
        churn_result: churnResult,
        customer_data: churnForm
      })
      
      // Gerçek XAI endpoint'ini çağır
      const response = await apiService.post('/api/xai/explain', {
        question: whyQuestion,
        churn_result: churnResult,
        customer_data: churnForm
      })
      ...
      if (response.explanation) {
        setWhyAnswer({
          ...
          lime_explanation: response.lime_explanation
        })
      } else {
        // Fallback to simple explanation
        const result = {
          title: "Model Açıklaması",
          content: `Model bu tahmini yaparken müşteri profilini analiz etti. ${whyQuestion} sorusuna göre, müşteri risk faktörleri değerlendirildi.`,
          confidence: churnResult.churn_probability
        }
        setWhyAnswer(result)
      }
    } catch (error) {
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* XAI - Model'e Sor Section - Sol tarafta */}
            {churnResult && (
              <Card className="animate-fade-up border-2 border-orange-200 dark:border-orange-800 bg-gradient-to-br from-orange-50/50 to-yellow-50/50 dark:from-orange-900/20 dark:to-yellow-900/20">
                <CardHeader className="text-center pb-4">
                  ...
                  <CardTitle className="text-2xl font-bold bg-gradient-to-r from-orange-600 to-yellow-600 bg-clip-text text-transparent">
                    Model'e Sor: "Neden Böyle Tahmin Yaptı?"
                  </CardTitle>
                  <CardDescription className="text-base">
                    Model tahminini sorgulayın ve detaylı açıklama alın
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="flex space-x-2">
                    <Input
                      placeholder="Neden bu müşteri için yüksek churn riski tahmin ettiniz?"
                      value={whyQuestion}
                    ...
                    <Button 
                      ...
                      disabled={isAskingWhy || !whyQuestion.trim()}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[
                      ...
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        onClick={() => setWhyQuestion(question)}
                  ...
                  {whyAnswer && (
                    <div className="p-4 bg-orange-100 dark:bg-orange-900/30 rounded-lg">
                      <h4 className="font-semibold text-orange-900 dark:text-orange-100 mb-2">Model'in Cevabı</h4>
