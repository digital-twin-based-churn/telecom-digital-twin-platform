...
# Spark session
# --------------------------
def get_spark(app_name: str = "Prepaid Churn Experiment") -> SparkSession:
...
def run_experiment(
    spark: SparkSession,
    df_raw: DataFrame,
    out_dir: str = "artifacts/prepaid_baseline",
    seed: int = 42,
) -> dict:
    out_dir = Path(out_dir).as_posix()
    Path(out_dir).mkdir(parents=True, exist_ok=True)

    # 1) Feature engineering (Prepaid-only)
    print("[checkpoint] starting feature engineering for Prepaid", flush=True)
    try:
        sample_frac_env = float(os.getenv("SAMPLE_FRAC", "0.0"))
    except Exception:
        sample_frac_env = 0.0
    df_feat, feat_cols = build_prepaid_features(df_raw, sample_frac=sample_frac_env)
    ...
    print("=== PREPAID REPORT ===", flush=True)
...
if __name__ == "__main__":
    ...
    run_experiment(spark, df_raw, out_dir=os.getenv("OUT_DIR", "artifacts/prepaid_baseline"))
    spark.stop()
