# experiment.py — Postpaid churn baseline with calibration (Spark ML)
# -------------------------------------------------------------------
# - Yalnız Postpaid (feature_engineering.build_postpaid_features)
# - Zaman bazlı (event_time) veya stratified split
# - Class imbalance -> weightCol
# - GBTClassifier (Spark native) + Isotonic calibration
# - ROC-AUC, PR-AUC, Brier, F1-optimal threshold, Precision@K (%1, %5)
# - Model ve kalibrasyon artefaktları kaydı (artifacts/postpaid_baseline)
...
# Spark session
# --------------------------
def get_spark(app_name: str = "Postpaid Churn Experiment") -> SparkSession:
    ...
    try:
        ...
        # In some versions it's a property, in others a method; normalize
        if callable(ui_url):
            ui_url = ui_url()
...
def parse_topk_list(env_value: str | None) -> List[float]:
    ...
    Kabul edilen format örnekleri:
    - "0.005,0.02,0.01,0.05"
    - "%0.5,%2" veya "0.5%,2%" (yüzde olarak)
    - Boş veya hatalıysa varsayılan döner.
    """
    default = [0.01, 0.05]
    if not env_value:
        return default
    ...
    for it in items:
        ...
        try:
            v = float(s)
            # Eğer 1'den büyükse yüzde girişi varsay (örn. 2 -> %2)
            if v > 1.0:
                v = v / 100.0
            parsed.append(v)
        except Exception:
...
def evaluate_basic(df_pred: DataFrame, label_col="label", score_col="score") -> dict:
    # Spark evaluator rawPredictionCol/double destekliyor; score'u double veriyoruz
...
def precision_at_k(df_pred: DataFrame, k_frac: float, label_col="label", score_col="score") -> float:
    ...
    Büyük veri setlerinde global sort yerine yaklaşık quantile eşiği ile
    (score >= q_{1-k}) filtreleyip precision hesaplar. Sınırda eşitlik durumları
    (çok sayıda aynı skor) için sonuç yaklaşık olabilir.
    """
    total = df_pred.count()
...
def export_topk_candidates(
    ...
    if total == 0:
        return
    # Kesin Top‑K: skor DESC sıralayıp row_number() <= k_rows seç
    for k in ks:
        ...
        # Dosya adı: k yüzdesini güvenli dizeye çevir (0.5% -> top_0_5pct.csv)
        ...
        # Önce Spark writer, olmazsa pandas fallback
        try:
            ...
            try:
                ...
                _pd.DataFrame(pdf).to_csv(os.path.join(Path(out_dir).as_posix(), f"top_{pct_str}pct_fallback.csv"), index=False)
...
def find_best_threshold(df_val: DataFrame, label_col="label", score_col="score") -> Tuple[float, dict]:
    # Daha sık grid: 0.80 - 0.995 aralığında 0.005 adım + birkaç sabit nokta
...
def save_isotonic_model(model: IsotonicRegressionModel, out_dir: str):
    # Windows uyumluluğu: Hadoop FS ile daha sağlıklı çalışması için POSIX yol
    out_dir_posix = Path(out_dir).as_posix()
...
def run_experiment(
    spark: SparkSession,
    df_raw: DataFrame,
    out_dir: str = "artifacts/postpaid_baseline",
    seed: int = 42,
) -> dict:
    # Çıktı dizinini POSIX'e çevir (Windows'ta Hadoop FS uyumu)
    out_dir = Path(out_dir).as_posix()
    Path(out_dir).mkdir(parents=True, exist_ok=True)

    # 1) Feature engineering (Postpaid-only)
    print("[checkpoint] starting feature engineering", flush=True)
    # Ortamdan küçük örnek çalışma için SAMPLE_FRAC (0.0 -> örnekleme yok)
    try:
        sample_frac_env = float(os.getenv("SAMPLE_FRAC", "0.0"))
    except Exception:
        sample_frac_env = 0.0
    df_feat, feat_cols = build_postpaid_features(df_raw, sample_frac=sample_frac_env)
    ...
    # 5) Train GBT
    # Ortamdan GBT hiperparametreleri
    ...
    # 5.1) (Opsiyonel) Küçük hiperparametre araması
    ...
    # 6) Raw scores (prob_1) for val & test (VectorUDT -> array)
    # id varsa koru
    ...
    # 6) Isotonic calibration (lighter, no row_id join)
    if has_id:
    ...
    # threshold seçimi: önce grouped F1, olmazsa approxQuantile
    try:
    ...
    # 8) Persist artifacts (isteğe bağlı olarak atlanabilir)
    model_dir = os.path.join(out_dir, "gbt_model")
    Path(out_dir).mkdir(parents=True, exist_ok=True)
    gbt_saved = False
    if os.getenv("SKIP_SAVE", "0") != "1":
        # GBT model kaydı: Windows'ta HADOOP_HOME/winutils yoksa hata verebilir → güvenli dene
        try:
            print(f"[checkpoint] saving GBT model to: {model_dir}", flush=True)
            model.write().overwrite().save(Path(model_dir).as_posix())
            gbt_saved = True
        except Exception as e:
            # Kaydetme atlandı; yine de kalibrasyon ve rapor yazılacak
            ...
            with open(warn_path, "w", encoding="utf-8") as wf:
                ...
                wf.write("Hint: On Windows, set HADOOP_HOME and provide winutils.exe to enable Hadoop FS saves.\n")
            print(f"[warn] GBT save skipped -> {warn_path}", flush=True)
        # Isotonic kalibrasyonu JSON + Spark model olarak yaz (saf Python IO + Spark ML)
        try:
    ...
    # 9) Operasyonel çıktılar: Top-K adaylar ve eşik bazlı confusion
    try:
...
if __name__ == "__main__":
    ...
    schema = T.StructType([
        ...
        # Opsiyonel: varsa daha iyi split
    ...
    run_experiment(spark, df_raw, out_dir=os.getenv("OUT_DIR", "artifacts/postpaid_baseline"))
    spark.stop()
