# feature_engineering.py  — Postpaid-only feature pipeline (Spark)
# ---------------------------------------------------------------
# - Yalnızca Postpaid kayıtları üzerinde çalışır
# - Winsor eşikleri ve log1p önerilerini "preprocessing_suggestions.csv" ya da "Postpaid.csv" dosyalarından okur
# - Dosya yoksa: Postpaid altkümesi üzerinde approxQuantile ile p01 / p50 / p99 hesaplar (tek tarama)
...
def _load_bounds_and_flags_from_suggestions(
    spark, segment_label: str = "postpaid"
) -> Tuple[Dict[str, Tuple[float, float, float]], Dict[str, bool]]:
    """
    preprocessing_suggestions.csv (ya da Postpaid.csv) içinden
    ...
    for path in SUGGESTIONS_PATHS:
        ...
        # alt set: yalnız postpaid satırlar
        if has_segment:
...
def _compute_quantiles_postpaid(
    df_postpaid: DataFrame, numeric_cols: List[str]
) -> Dict[str, Tuple[float, float, float]]:
    """Tek taramada Postpaid altkümesi için p01/p50/p99 hesapla."""
    if not numeric_cols:
        return {}
    num_df = df_postpaid.select(*[F.col(c).cast("double").alias(c) for c in numeric_cols])
...
# === Asıl fonksiyon: yalnız Postpaid ===

def build_postpaid_features(
    df: DataFrame,
    sample_frac: float = 0.0,
    suggestions_prefer_segment: bool = True,
) -> Tuple[DataFrame, List[str]]:
    """
    Postpaid kayıtları için feature engineering uygular ve (df, feature_list) döner.

    Notlar:
    - Winsor eşikleri ve log1p seçimi: preprocessing_suggestions.csv / Postpaid.csv öncelikli.
      Bulunamazsa approxQuantile(Postpaid) ile p01/p50/p99 hesaplanır.
    - Eksik stratejisi:
        * Yapısal NA (ör. call/roaming bazı satırlarda NA): 0 + *_missing flag
        * Operasyonel NA (monthly_charge, data_usage): medyan (Postpaid p50) + *_missing flag
        * auto_payment: {True=1, False=0, None=None} + auto_payment_missing
    """

    # 1) Yalnız Postpaid altkümesi
    df = df.filter(F.col("service_type") == F.lit("Postpaid"))
    ...
    df = (
        df.withColumn("avg_call_duration_missing", F.when(F.col("avg_call_duration").isNull(), 1.0).otherwise(0.0))
          .withColumn("call_drops_missing",       F.when(F.col("call_drops").isNull(),       1.0).otherwise(0.0))
          .withColumn("data_usage_missing",       F.when(F.col("data_usage").isNull(),       1.0).otherwise(0.0))
    ...
    # 5) İhtiyaç duyacağımız sayısal kolonlar (ham)
    numeric_candidates = [
        "avg_call_duration",
        "call_drops",
        "roaming_usage",
        ...
        "overdue_payments",
        "satisfaction_score",
        "avg_top_up_count",  # postpaid'de çoğunlukla 0 ama yine de dahil
    ]
    numeric_cols_present = [c for c in numeric_candidates if c in df.columns]

    # 6) İPUCU DOSYALARINDAN WINSOR / LOG1P BİLGİSİ
    spark = df.sql_ctx.sparkSession
    quantiles_sug, log1p_flags = _load_bounds_and_flags_from_suggestions(spark, segment_label="postpaid")

    # Eğer birleşik 'preprocessing_suggestions.csv' geldiyse oradaki segment bazlı değerlere öncelik verelim
    # segment dosyası yoksa ya da ilgili kolon bulunamadıysa approxQuantile ile hesapla:
    need_compute = []
    p50_for_fill: Dict[str, Optional[float]] = {}

    if suggestions_prefer_segment and quantiles_sug:
        for c in numeric_cols_present:
            p = quantiles_sug.get(c) or quantiles_sug.get(c.lower()) or quantiles_sug.get(c.upper())
            if p:
                p01, p50, p99 = p
                p50_for_fill[c] = p50
            else:
        ...
        need_compute = numeric_cols_present

    if need_compute:
        # Tek taramada Postpaid altkümesi üzerinden p01/p50/p99
        q_post = _compute_quantiles_postpaid(df, need_compute)
        for c in need_compute:
            p01, p50, p99 = q_post.get(c, (None, None, None))
            quantiles_sug[c] = (p01, p50, p99)
            p50_for_fill[c] = p50

    # 7) Eksik değer doldurma
    # Yapısal NA: call_drops / avg_call_duration / roaming_usage -> 0 + flag (zaten flag var)
    struct_zero_fill = ["avg_call_duration", "call_drops", "roaming_usage"]
    for c in struct_zero_fill:
        if c in df.columns:
            df = df.withColumn(c, F.when(F.col(c).isNull(), 0.0).otherwise(F.col(c).cast("double")))

    # Operasyonel NA: data_usage / monthly_charge -> medyan (Postpaid p50) + flag
    ...
    for c in numeric_cols_present:
        if c not in struct_zero_fill and c not in ["data_usage", "monthly_charge"]:
            med = p50_for_fill.get(c)
            if med is not None:
                df = df.withColumn(c, F.when(F.col(c).isNull(), float(med)).otherwise(F.col(c).cast("double")))
            else:
    ...
    safe_minutes = F.greatest(F.col("avg_call_duration").cast("double"), F.lit(1.0))
    ...
    df = df.withColumn("drops_per_min",        _safe_div("call_drops", "avg_call_duration", eps=1.0))
    ...
    for extra in ["monthly_charge", "data_usage", "customer_support_calls", "avg_top_up_count"]:
    ...
    feature_cols: List[str] = [
        ...
        "monthly_charge", "overdue_payments", "satisfaction_score", "avg_top_up_count",
        # NA bayrakları
        "avg_call_duration_missing", "call_drops_missing", "data_usage_missing",
        "monthly_charge_missing", "auto_payment_missing",
        # engineered oranlar
        "overdue_ratio", "support_calls_rate", "charge_per_gb",
        "drops_per_min", "usage_per_tenure", "charge_per_tenure",
        # bayraklar
        "has_overdue_flag", "satisfaction_low", "has_any_app",
        # hafif etkileşim
